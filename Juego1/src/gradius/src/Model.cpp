...
vec4 Model::AABBWithOffset (mat4 offset) {
	vec4 tmp = offset * vec4(massCenter, 1.0f);
	return vec4(tmp.x + AABB.x, tmp.x + AABB.y, tmp.y + AABB.z, tmp.y + AABB.w);
}
...
void Model::processNode (aiNode *node, const aiScene *scene) {
	...
	for ( unsigned int i = 0; i < node->mNumMeshes; i++ ) {
		...
		massCenter += meshes[i].massCenter;
		++numberMeshes;

		if ( meshes[i].AABB.x < AABB.x ) // left
			AABB.x = meshes[i].AABB.x;
		if ( meshes[i].AABB.y > AABB.y ) // right
			AABB.y = meshes[i].AABB.y;
		if ( meshes[i].AABB.z > AABB.z ) // up
			AABB.z = meshes[i].AABB.z;
		if ( meshes[i].AABB.w < AABB.w ) // down
			AABB.w = meshes[i].AABB.w;
	}
